...
#### 设计理念
- 优化视频抽取规则，确保每个场景文件夹只随机抽取一个时长符合要求的视频
- 改进视频裁剪逻辑，从视频开头裁剪而不是从中间部分
- 明确重编码和快速不重编码两种模式的优劣势
...
#### 优化策略
- 视频抽取：优先选择时长大于等于音频时长的视频，避免短视频拼接
- 视频裁剪：统一从视频开头裁剪，便于素材的准备和预览
- 避免重复：跟踪已使用的视频路径，避免在不同场景中重复使用相同视频
- 编码模式：提供重编码和快速不重编码两种模式的详细说明，便于用户选择合适的处理方式

#### 注意事项
- 随机抽取：每个文件夹中仍然随机抽取视频，但确保视频时长符合要求
- 裁剪规则：从视频开头开始裁剪，裁剪后的视频时长与音频时长一致
- 编码选择：重编码模式兼容性更好但速度较慢，快速不重编码模式速度更快但可能与某些特效不兼容

### 编码模式对比

#### 重编码模式
- **实现原理**：先通过MoviePy处理视频，再使用FFmpeg进行编码
- **优势**：
  - 更高的兼容性和稳定性
  - 支持更丰富的视频处理效果
  - 更适合复杂的视频特效和转场
...
#### 快速不重编码模式
- **实现原理**：直接使用FFmpeg硬件加速编码，跳过MoviePy的编码流程
- **优势**：
  - 处理速度更快，通常快2-5倍
  - 减少视频质量损失
  - 更高效利用GPU资源
...
@design.md 目标路径: src/core/video_processor.py 操作类型: 修改 更新内容: 
- 设计理念: 
  * 优化视频抽取规则，确保每个场景文件夹只随机抽取一个时长符合要求的视频
  * 改进视频裁剪逻辑，从视频开头裁剪而不是从中间部分
  * 明确重编码和快速不重编码两种模式的优劣势
...
- 核心实现: 
  ...
  ```
- 优化策略: 
  * 详细文档：为用户提供清晰的技术说明，帮助用户根据需求选择合适的编码模式
  * 性能与质量平衡：提供不同编码选项，让用户可以根据设备性能和视频质量需求进行选择
- 注意事项: 
